...
export default function BusinessCard() {
  ...
  const generateVCard = () => {
    ...
    a.href = url;
    const fileName = (user.fullName || 'contact').replace(/\s+/g, '_');
  ...
  useEffect(() => {
    document.title = loading ? 'Loading Business Card...' : 
                     (user ? `${user.fullName || user.displayName || 'Unknown User'} - Business Card` : 
                     'Card Not Found - NFC Digital Profile');
  }, [loading, user]);
